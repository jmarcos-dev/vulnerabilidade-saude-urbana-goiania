...
Configuração via .env na raiz do projeto (NÃO versionar este arquivo):
    DB_HOST=localhost
    DB_PORT=5432
    DB_NAME=vulnerabilidade_saude
    DB_USER=seu_usuario
...
if __name__ == "__main__":
    df = carregar_dados()
    tabela_mensal = resumo_mensal(df)
    medias(df, tabela_mensal)

    # Exporta a série mensal para juntar depois com o csv de clima
    # (o csv de clima precisa ter uma coluna de data que dê pra agregar
    # no mesmo grão de "ano_mes" antes do merge)
    saida = PASTA_SCRIPT / "casos_mensais_goiania.csv"
